...
function App() {
  ...
  const [todoStatus, setTodoStatus] = useState<{ uncompleted: number; completed: number } | null>(null);
    ...
    try {
      ...
      if (latestReflection) {
        ...
        if (analysisData?.suggestedTodos) {
          const completed = (analysisData.suggestedTodos as SuggestedTodo[]).filter((t: SuggestedTodo) => t.isCompleted).length;
  ...
  const handleToggleTodo = async (todoId: string, currentStatus: boolean) => {
    try {
      ...
      setHistory(prevHistory => prevHistory.map(reflection => {
