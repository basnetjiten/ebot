...
import { HiSun, HiPlus, HiCheck, HiDesktopComputer, HiEmojiHappy } from 'react-icons/hi';
        ...
        <aside className="w-80 bg-white/80 backdrop-blur-xl border-r border-slate-200/60 flex flex-col hidden lg:flex h-screen sticky top-0 relative z-20">
            <div className="p-8 border-b border-slate-100/50">
                <div className="flex items-center gap-3">
                    ...
                    <h1 className="font-black text-2xl bg-clip-text text-transparent bg-gradient-to-r from-slate-800 to-slate-600 tracking-tight">Reflectly</h1>
                </div>
                <p className="text-[10px] uppercase tracking-[0.2em] font-bold text-indigo-500 mt-2 ml-1">AI Reflection Companion</p>
            </div>
            ...
            <div className="p-6">
                <nav className="space-y-2">
                    <Link
                        to="/"
                        className={`w-full flex items-center gap-3 px-4 py-3.5 rounded-2xl transition-all duration-300 group ${location.pathname === '/'
                            ? 'bg-gradient-to-r from-indigo-500 to-purple-600 text-white shadow-md shadow-indigo-500/25'
                            : 'text-slate-500 hover:bg-slate-50 hover:text-slate-700'
                            }`}
                    >
                        <HiSun size={20} className={location.pathname === '/' ? 'text-white/90' : 'group-hover:text-amber-500 transition-colors'} />
            ...
            <div className="flex-1 overflow-y-auto px-6 py-4 custom-scrollbar space-y-8">
                    ...
                    >
                        <div className="space-y-3">
                            <h3 className="text-[10px] font-black uppercase tracking-widest text-slate-400 flex items-center gap-2">
                                <span className="w-1 h-1 rounded-full bg-indigo-500"></span> Current Topics
                            </h3>
                            <div className="flex flex-wrap gap-2">
                                {analysisResult.keywords?.map((keyword, i) => (
                                    <span key={i} className="px-3 py-1 bg-white border border-slate-200/60 rounded-full text-xs font-medium text-slate-600 shadow-sm hover:shadow-md hover:scale-105 transition-all cursor-default">
                                        {keyword}
                                    </span>
                        ...
                        <div className="space-y-3">
                            <h3 className="text-[10px] font-black uppercase tracking-widest text-slate-400 flex items-center gap-2">
                                <span className="w-1 h-1 rounded-full bg-purple-500"></span> Current Summary
                            </h3>
                        ...
                        {analysisResult.suggestedTodos && analysisResult.suggestedTodos.length > 0 && (
                            <div className="space-y-3">
                                <h3 className="text-[10px] font-black uppercase tracking-widest text-slate-400 flex items-center gap-2">
                                    <span className="w-1 h-1 rounded-full bg-emerald-500"></span> Action Items
                                </h3>
                                <div className="space-y-2">
                                    {analysisResult.suggestedTodos.map((todo) => (
                                        <div
                                            key={todo.id}
                                            className={`group flex items-center gap-3 p-3 bg-white/60 border rounded-2xl shadow-sm transition-all backdrop-blur-sm ${todo.isCompleted ? 'border-emerald-100 bg-emerald-50/30' : 'border-slate-100 hover:border-indigo-200 hover:shadow-md hover:-translate-y-0.5'
                                                }`}
                                        >
                                            <button
                ...
                {userKeywords.length > 0 && (
                    <div className="space-y-3">
                        <h3 className="text-[10px] font-black uppercase tracking-widest text-slate-400">Popular Topics</h3>
                        <div className="flex flex-wrap gap-2">
                            {userKeywords.slice(0, 5).map((kw, i) => (
                                <div key={i} className="px-3 py-1.5 bg-gradient-to-r from-sky-50 to-indigo-50 border border-indigo-100/50 rounded-full text-xs text-indigo-700 font-bold flex items-center gap-1.5 shadow-sm">
                                    {kw.keyword}
                                    <span className="bg-white/80 px-1.5 py-0.5 rounded text-[10px] shadow-sm">{kw.count}</span>
                                </div>
                            ))}
                        </div>
                    </div>
                ...
                {history.length > 0 && (
                    <div className="space-y-3">
                        <div className="flex items-center justify-between">
                            ...
                            <Link to="/dashboard" className="text-[10px] font-bold text-indigo-600 hover:text-indigo-700 hover:underline uppercase tracking-tight">
                                View All
                            </Link>
                        </div>
                        <div className="space-y-2">
                            {history.slice(0, 3).map((item) => (
                                ...
                                >
                                    <div className={`absolute top-0 left-0 w-1 h-full ${item.type === 'morning' ? 'bg-amber-400' : 'bg-indigo-500'} opacity-0 group-hover:opacity-100 transition-opacity`} />
                                    <div className="flex items-center justify-between mb-1 pl-2">
                                        <span className={`text-[9px] font-black uppercase tracking-widest ${item.type === 'morning' ? 'text-amber-500' : 'text-indigo-500'}`}>
                                            {item.type}
                                        </span>
            ...
            <div className="p-6 border-t border-slate-100/50 mt-auto space-y-3 bg-white/30 backdrop-blur-md">
                <div className="bg-white/60 rounded-xl p-3 border border-slate-100/50 mb-2 shadow-sm backdrop-blur-sm">
                    <p className="text-[9px] font-black uppercase tracking-wider text-slate-400 mb-1">Logged in as</p>
