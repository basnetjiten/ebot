...
import { HiSun, HiMoon, HiArrowLeft, HiCalendar, HiTag, HiOutlineDocumentText, HiCheck, HiTrash } from 'react-icons/hi';
        ...
        <div className="flex-1 bg-gradient-to-br from-slate-50 via-indigo-50/20 to-rose-50/20 min-h-screen p-8 overflow-y-auto relative">
            ...
            <header className="mb-12 relative z-10">
                <div className="flex items-center gap-4 mb-2">
                    ...
                    <div>
                        ...
                        <p className="text-slate-500 font-medium">Explore your past insights and daily growth.</p>
                    </div>
                </div>
            </header>
            ...
            >
                {history.map((reflection) => (
                    ...
                    >
                        ...
                        <div className={`absolute -top-20 -right-20 w-48 h-48 rounded-full blur-3xl opacity-20 group-hover:opacity-40 transition-opacity duration-500 ${reflection.type === 'morning' ? 'bg-amber-400' : 'bg-indigo-600'}`} />

                        <div className="flex items-center justify-between mb-6 relative z-10">
                            <div className={`p-3 rounded-2xl transition-transform group-hover:scale-110 duration-300 ${reflection.type === 'morning'
                                ? 'bg-amber-50 text-amber-500 ring-1 ring-amber-100'
                                : 'bg-indigo-50 text-indigo-500 ring-1 ring-indigo-100'}`}>
                                {reflection.type === 'morning' ? <HiSun size={24} /> : <HiMoon size={24} />}
                            </div>
                            <div className="flex items-center gap-3">
                                <span className="px-3 py-1 rounded-full bg-white/80 text-[10px] font-bold text-slate-400 uppercase tracking-widest shadow-sm border border-slate-100">
                                    {new Date(reflection.timestamp).toLocaleDateString([], { month: 'short', day: 'numeric' })}
                                </span>
                                <button
                                    onClick={(e) => {
                                        e.preventDefault();
                        ...
                        <h3 className="text-xl font-black text-slate-800 mb-3 group-hover:text-indigo-600 transition-colors capitalize tracking-tight flex items-center gap-2">
                            {reflection.type == 'morning' ? 'Morning Intention' : 'Evening Reflection'}
                            <HiArrowLeft className="opacity-0 -ml-4 group-hover:opacity-100 group-hover:ml-0 transition-all text-indigo-400 rotate-180" size={16} />
                        </h3>
                        ...
                        {reflection.keywords && reflection.keywords.length > 0 && (
                            <div className="flex flex-wrap gap-2 pt-6 border-t border-slate-100/50 relative z-10">
                                {reflection.keywords.slice(0, 3).map((kw, i) => (
                                    <span key={i} className="px-2.5 py-1 bg-white border border-slate-100 rounded-lg text-slate-500 text-[10px] font-bold uppercase tracking-wider shadow-sm group-hover:border-indigo-100 transition-colors">
                                        {kw}
                                    </span>
                                ))}
                            </div>
            ...
            {history.length === 0 && (
                <div className="flex flex-col items-center justify-center py-40 text-center relative z-10">
                    ...
                    <p className="text-slate-500 max-w-sm mt-2 text-lg">Start your journey by completing your first daily entry!</p>
                </div>
            )}
                    ...
                    <>
                        ...
                        >
                            <div className="flex flex-col items-center text-center mb-8">
                                ...
                                <h3 className="text-2xl font-black text-slate-800 mb-2">Delete Reflection?</h3>
                                <p className="text-slate-500 leading-relaxed max-w-xs">
                                    This will permanently delete this reflection and all associated action items.
                                </p>
                            </div>
                    ...
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-6 sm:p-12">
                        ...
                        >
                            ...
                            <div className="p-8 pb-0 flex items-center justify-between relative z-10">
                                ...
                                <div className={`px-5 py-2 rounded-full text-xs font-black uppercase tracking-[0.2em] shadow-sm ${selectedReflection.type === 'morning' ? 'bg-amber-50 text-amber-600 ring-1 ring-amber-100' : 'bg-indigo-50 text-indigo-600 ring-1 ring-indigo-100'
                                    }`}>
                            ...
                            <div className="p-10 overflow-y-auto custom-scrollbar">
                                ...
                                <h2 className="text-4xl font-black text-slate-800 mb-12 tracking-tight">Detailed Insights</h2>

                                <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
                                    <div className="lg:col-span-2 space-y-12">
                                        <section>
                                            <h4 className="text-xs font-black uppercase tracking-widest text-indigo-500 mb-6 flex items-center gap-2">
                                                <span className="w-8 h-[2px] bg-indigo-500 rounded-full" /> Your Entry
                                            </h4>
                                        ...
                                        {selectedReflection.summary && (
                                            <section>
                                                <h4 className="text-xs font-black uppercase tracking-widest text-purple-500 mb-6 flex items-center gap-2">
                                                    <span className="w-8 h-[2px] bg-purple-500 rounded-full" /> AI Summary
                                                </h4>
                                            ...
                                            <section className="bg-slate-50/50 p-6 rounded-3xl border border-slate-100/50">
                                                ...
                                                <div className="space-y-4">
                                                    {selectedReflection.suggestedTodos.map((todo) => (
                                                        <div key={todo.id} className="flex items-start gap-3 p-4 bg-white rounded-2xl border border-slate-100 shadow-sm group/todo hover:border-emerald-200 transition-colors">
                                                            <button
                                                                onClick={(e) => {
                                                                    e.stopPropagation();
                                                                    onToggleTodo(todo.id, todo.isCompleted);
                                                                }}
                                                                className={`mt-1 shrink-0 w-6 h-6 rounded-lg border flex items-center justify-center transition-all ${todo.isCompleted ? 'bg-emerald-500 border-emerald-500 text-white' : 'bg-white border-slate-200 hover:border-emerald-400'
                                                                    }`}
                                                            >
                                                                {todo.isCompleted && <HiCheck size={14} />}
                                                            </button>
                                                            <div>
                                                                <p className={`text-sm font-bold transition-all ${todo.isCompleted ? 'text-slate-400 line-through' : 'text-slate-700'}`}>
                                                                    {todo.title}
                                                                </p>
                                                            </div>
                                                        </div>
                                            ...
                                            <section>
                                                ...
                                                <div className="flex flex-wrap gap-2">
                                                    {selectedReflection.keywords.map((kw, i) => (
                                                        <span key={i} className="px-4 py-2 bg-gradient-to-r from-slate-50 to-white border border-slate-200 rounded-xl text-xs font-bold text-slate-600 shadow-sm">
                                                            {kw}
                                                        </span>
                                                    ))}
                                                </div>
